...
commandCreatorSame() {
	...
	for credential in ${ip_addresses[@]}; do
		while IFS= read -r ip_addr ; do
			command="sshpass -p '$password' ssh $username@$ip_addr 'bash -s' < sessionHandler.sh ;"
			commands+=("$command")
			((number_of_commands++))
		done<<< $credential
	done
		...
		'y'|'Y')	# -> Has root priviliges
			...
			for credential in ${credentials[@]}; do
				while IFS=', ' read -r val1 val2 val3 ; do
					command="sshpass -p '$val3' ssh $val2@$val1 'bash -s' < sessionHandler.sh ;"
					commands+=("$command")
					((number_of_commands++))
				done<<< $credential
			done
